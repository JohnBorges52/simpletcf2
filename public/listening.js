...
(async () => {
  ...
  async function confirmAnswer(q) {
    ...
    if (!state.realTestMode) {
      trackListeningUsage().catch(err => {
        console.error("Failed to track listening usage:", err);
      });
    }
      ...
      if (answeredCount > currentUsage && answeredCount > 0) {
        ...
        await window.SubscriptionService.updateUserSubscriptionData(user.uid, {
          usage: {
            ...
            listeningQuestionsAnswered: answeredCount
          }
        });
  ...
  /**
   * Check if user has access to listening practice
  ...
  function applyTierRestrictions() {
    ...
    const realTestBtn = RT.realTestBtn();
  ...
  async function init() {
    ...
    const hasAccess = await checkListeningAccess();
    if (!hasAccess) {
